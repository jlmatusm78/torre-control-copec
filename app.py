...
st.set_page_config(
    page_title="Torre de Control COPEC",
    page_icon="🚦",
    layout="wide",
...
@media (prefers-color-scheme: dark) {
    :root {
        ...
        --line: #334155;
        --primary: #38BDF8;
        --soft-primary: rgba(56,189,248,.10);
...
def risk_score(data):
    ...
    if unique_drivers > 0:
        top_share = data["Conductor"].value_counts(normalize=True).iloc[0]
        concentration_component = min(top_share / 0.35, 1) * 15
...
def executive_insight(data):
    ...
    return (
        ...
        f"Fatiga/Somnolencia: **{len(fat)} eventos**, "
        f"**{int((fat[CUMPL_COL] == 'SI').sum())} cumple** y "
        f"**{int((fat[CUMPL_COL] == 'NO').sum())} no cumple**."
    )
...
st.caption(
    "Dashboard operativo de alertas Guardian y FlotaGo · Fuente: Google Sheets · "
    "Datos desde 01-01-2026 · Refresco cada 5 minutos."
)
...
with c1:
    metric_card("Total alertas", f"{len(filtered):,}".replace(",", "."), "Eventos filtrados")
with c2:
    metric_card("Plataformas", filtered["Plataforma"].nunique(), "Con alertas")
with c3:
    metric_card("Transportistas", filtered["Transportista"].nunique(), "Con alertas")
with c4:
...
with c7:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Riesgo</div>'
        f'<div class="metric-value"><span class="{css_class}">{level}</span></div>'
        f'<div class="metric-note">Score {score}/100</div></div>',
        unsafe_allow_html=True,
    )
...
with g4:
    ...
    fatigue_bar = pd.DataFrame({
        "Estado": ["Cumple", "No cumple"],
        "Eventos": [cumple, no_cumple],
    })
    fig = px.bar(
        fatigue_bar,
        x="Estado",
...
with g5:
    ...
    pa = (
        filtered.groupby(["Plataforma", "Incidente"])
        .size()
...
with tab3:
    if fatigue.empty:
        st.info("No hay eventos de fatiga/somnolencia para los filtros seleccionados.")
        fs = pd.DataFrame()
    else:
        fs = (
            fatigue[fatigue[CUMPL_COL].isin(["SI", "NO"])]
            .groupby(["Plataforma", "Conductor"])[CUMPL_COL]
            .agg(
                Eventos="count",
                Cumple=lambda s: int((s == "SI").sum()),
                No_cumple=lambda s: int((s == "NO").sum()),
            )
            .reset_index()
...
with tab4:
    ps = (
        filtered.groupby("Plataforma")
        .agg(
            Alertas=("Incidente", "count"),
            Transportistas=("Transportista", "nunique"),
            Conductores=("Conductor", "nunique"),
            Fatiga_Somnolencia=("EsFatiga", "sum"),
        )
        .reset_index()
    )
...
excel_bytes = to_excel_bytes({
    ...
    "Fatiga Somnolencia": fs if "fs" in locals() else pd.DataFrame(),
    "Resumen plataforma": ps if "ps" in locals() else pd.DataFrame(),
    "Datos filtrados": filtered[
        [
            c for c in [
                "Fecha",
                "Plataforma",
...
st.download_button(
    "Descargar Excel con resultados filtrados",
    data=excel_bytes,
    file_name=f"torre_control_copec_{start_date}_a_{end_date}.xlsx",
